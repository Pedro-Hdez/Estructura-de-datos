#ifndef GRAFICAS3_H_INCLUDED
#define GRAFICAS3_H_INCLUDED
...
void lista_arcos::pintar(){
    ...
    while(p){ //Se imprimen todos los valores que forman parte de la estructura.
        std::cout << p -> numNodo << "-" << p->direccionNodo->numNodo << " Longitud: " << p->longitudArco << " ; ";
        p = p -> siguiente;
    }
    std::cout << "\b\b ";
}
...
void lista::pintar(){
    ...
    while(p){
        std::cout << p -> direccionNodo << "Longitud: " << p->longitud << " , ";
        p = p -> antes;
    }
    std::cout << "\b\b ";
...
void grafica::agregar_arco(int a, int b, float l){
    ...
    ((q->entrantes).lugar_agregado())->longitudArco = l;
}
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
